...
**Local carrying-capacity grounding and the theoretical case for a *dynamic* ECC.** Carrying-capacity-specific ML already exists in forestry tourism (multi-source GAT-Transformer, Ma & Geng, MDPI *Forests* 17(5):534, 2026) and in a landscape-driver study of trail formation using slope, elevation, and the **Topographic Wetness Index** as explainable-ML covariates (Guo et al., *Land* 15(5):715, 2026) — the latter is a direct precedent for Module 4's own use of DEM-derived TWI as a node feature, since it shows TWI validated as a covariate for trail/hiker spatial behaviour rather than only as a hydrology metric. Explainable tourist-arrival drivers via XGBoost + SHAP (Istanbul, Emerald *JHTT*, 2025) and, replacing the previously-unverifiable RG 392533688 claim, Sri Lanka-specific hybrid ML tourism-demand forecasting comparing SVR/Random Forest/ANN against a SARIMA baseline with social-media-sentiment integration (Hewapathirana, *Journal of Tourism Futures* 11(2):261, 2023/2025) inform the temporal/seasonal feature design. Beyond Sri Lanka, mountain-tourism carrying-capacity pressure is an active South Asian problem, not a uniquely local one: a 2025 multi-criteria carrying-capacity study of the Char Dham Himalayan pilgrimage/trekking circuit in Uttarakhand, India (Kuniyal et al., *Scientific Reports* 15:36340, 2025), documents visitor growth from ~1 million/year in the early 2000s to a record 5 million in 2023 and derives per-site daily caps from 21 geoscientific/bioscientific/socioeconomic/cultural indicators via AHP weighting — a different method from the Cifuentes cascade Module 4 uses, but strong problem-framing precedent that South Asian mountain-trail crowding is a live, high-stakes policy question. A 2025 systematic review of carrying-capacity methodologies generally (DuPuy et al., *Integrated Environmental Assessment and Management* 21(3):526–539, 2025) confirms the Cifuentes cascade remains an actively-cited, current-best-practice tool alongside ecological-footprint analysis, system dynamics, and GIS approaches — supporting evidence that citing the 1992 original as this proposal's foundational method is citing something still current, not superseded.

The case for *why* a dynamic, condition-responsive capacity number is preferable to a fixed permit quota rests on the **Limits of Acceptable Change (LAC)** framework (Stankey et al., USDA Forest Service GTR-INT-176, 1985 — **foundational/classical citation, cited for the named LAC method itself, on the same basis as the Cifuentes 1992 exception already used for the ECC cascade**), which reframes management around monitoring condition indicators against thresholds rather than capping visitor counts outright; a recent, directly on-domain application pairs this foundational cite with an empirical case — trail erosion assessed against Thresholds of Potential Concern and LAC in a high-visitation national park, using trail width as the impact indicator (Dragovich & Bajpai, *Sustainability* 14(7):4291, 2022). Together these support the proposal's own framing (echoed from the project's early scoping notes): a fixed daily quota cannot distinguish 50 hikers in a monsoon downpour from 300 on a dry day, whereas a condition/threshold-based approach — which a dynamically forecast ECC operationalises — can. **Critique:** the Char Dham study is the only source in this strand from a genuinely comparable mountain-trekking setting, and it uses a different (MCDA/AHP) method rather than Cifuentes — it is cited for problem-framing, not methodological precedent, and that distinction should not be blurred. **Gap:** none of the above operates on a *micro-spatial wilderness* graph offline; Module 4 adapts urban/regional ST-GNN methodology, small/sparse-graph baseline discipline, and synthetic-target precedent into that combination for the first time on a Sri Lankan trail network.

**Transfer-learning precedent for prototyping on large benchmarks before applying to a tiny target graph.** Module 4's plan (§5.5, Phase 5) is to prototype the ST-GNN architecture on METR-LA/PEMS-BAY/PeMS04/PeMS08 before training on the ~25–40 node Horton Plains graph — a methodology choice that, until this literature pass, had no direct citation grounding beyond "these are the standard benchmarks." Ada-STGCN (Yao et al., *IEEE Transactions on Intelligent Transportation Systems* 24(8):8592–8605, 2023) closes that gap directly: it combines a GCN+GRU spatio-temporal backbone with adversarial domain adaptation specifically to transfer a model trained on a data-**sufficient source** road network to a data-**scarce target** network, learning domain-invariant features to bridge the gap between them. This is a direct methodological precedent for training on a large, well-instrumented benchmark first and adapting to a small, sparse target graph second, rather than training on the target graph alone from scratch. **Critique:** Ada-STGCN still transfers between two real, road-traffic networks of the same domain and modality; Module 4's transfer is a larger jump (real urban traffic → synthetic wilderness-trail occupancy), so the precedent supports the *strategy* (source-rich pretraining before scarce-target fine-tuning) without guaranteeing the *domain gap* closes as cleanly — worth stating as a limitation, not glossing over.

**Uncertainty quantification for a forecast that feeds a real management decision.** A point forecast of occupancy is not, by itself, an adequate basis for a capacity decision that affects park access — the forecast should carry some notion of confidence before the Cifuentes cascade converts it into a number a manager acts on. Three recent, peer-reviewed sources address this directly in the ST-GNN/spatio-temporal setting: UQGNN (Yu et al., ACM SIGSPATIAL '25, 2025) jointly predicts expected values and uncertainty across multiple correlated spatiotemporal outputs from one architecture; a deep-ensemble Bayesian approach quantifies both data- and model-uncertainty in ST-GNN traffic forecasting (Mallick et al., *IEEE Transactions on Intelligent Transportation Systems* 25(8):9141–9152, 2024); and CONTINA (Yang et al., *Transportation Research Part C* 184:105502, 2025/2026) produces distribution-free confidence intervals with a formal coverage guarantee via conformal prediction, avoiding a Gaussian-error assumption that would likely be wrong for a low-count, over-dispersed wilderness-trail series. A related but architecturally distinct precedent, DG-STMTL (Cui et al., arXiv:2504.07822, 2025 — **preprint, peer-review status unconfirmed, flagged explicitly rather than presented as reviewed**), jointly forecasts multiple correlated traffic quantities from a shared dynamic-graph backbone via task-specific gating, structurally similar to how Module 4's own pipeline could in principle forecast occupancy and a derived risk signal together rather than as two separate models. **Critique:** none of these four papers touches carrying-capacity or wilderness settings — the citation supports "quantify uncertainty before it drives a decision" as a general spatio-temporal-forecasting best practice, not a domain-specific one; adopting a conformal-prediction or ensemble approach for Module 4's own ECC output remains future work rather than something already scoped into the MVP, and should be stated as such rather than implied as already planned.
...
| Yao et al. (Ada-STGCN), *IEEE TITS* 2023 | Urban road traffic | Large source → scarce target (transfer) | Real, both domains | No | Precedent for pretraining on large benchmarks then adapting to a data-scarce target graph |
| Yu et al. (UQGNN), ACM SIGSPATIAL 2025 | Urban multivariate ST prediction | Multiple real networks | Real | No | Joint mean + uncertainty prediction — precedent for not treating a point forecast as sufficient |
| Mallick et al., *IEEE TITS* 2024 | Urban traffic | Large | Real | No | Deep-ensemble uncertainty quantification for ST-GNN forecasts |
| Yang et al. (CONTINA), *Transp. Res. Part C* 2025/2026 | Urban traffic demand | Large | Real | No | Distribution-free confidence intervals via conformal prediction — fits a low-count, over-dispersed target better than a Gaussian assumption |
| **Module 4 (this proposal)** | **Wilderness trail** | **Very small (25–40 nodes)** | **Synthetic, calibration-validated** | **Yes — mandatory, by design** | Combines small-graph baseline discipline + synthetic-target precedent + domain-specific covariates + literature-grounded transfer strategy, offline |

**What the table shows that the prose alone does not:** only two of the twenty precedent sources (Gupta et al.; Zhang et al.) actually test a graph model against a non-graph baseline, and neither is set in a small, offline, single-domain wilderness network. Module 4 is the only row that is simultaneously very-small-graph, synthetic-ground-truth, and baseline-tested — which is the precise, falsifiable combination §5.5 commits to reporting honestly rather than assuming. The three uncertainty-quantification rows are not yet part of Module 4's own MVP (see the uncertainty-quantification paragraph's critique above) — they are included to show the methodological ceiling this project could grow toward, not a claim of what the MVP already does.
